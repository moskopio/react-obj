...
uniform bool uUseCellShading;
uniform float uCellSegments;
uniform float uCellAA;
...
vec3 cellShading(in float toLightNormal, in float viewAngleNormal) {
  float adjustedIntensity = max(1.0, SPECULAR_MAX - uSpecularIntensity);
  float diffuseShade = max(0.0, smoothCeil(toLightNormal * uCellSegments, uCellAA) / uCellSegments);
  float specularShade = smoothstep(SPECULAR_STEP, SPECULAR_STEP + uCellAA, pow(viewAngleNormal, adjustedIntensity));
...
void main() {
  ...
  color = mix(color, cellShadedColor, float(uUseCellShading));
