...
vec3 getCellShading(in ShadingCommon common, in Light light, in CellShading cell) {
  ...
  float fresnelShade = max(0.0, light.fresnel.intensity - smoothCeil(viewAngleNormal * segments, aa) / segments);
  ...
  vec3 fresnel = fresnelShade * light.fresnel.color * float(light.fresnel.enabled);
